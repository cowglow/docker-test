...
gulp.task("clean", (done) => {
  del([dirs.archive, dirs.dist]).then(() => {
    done();
  });
});
...
gulp.task("modernizr", (done) => {
  ...
  modernizr.build(modernizrConfig, (code) => {
    fs.writeFile(
      `${dirs.dist}/js/vendor/modernizr-${pkg.devDependencies.modernizr}.min.js`,
      code,
      done
    );
  });
...
gulp.task("lint:js", () =>
  gulp
    .src([`${dirs.src}/js/*.js`, `${dirs.test}/*.js`])
    .pipe(plugins().eslint())
...
gulp.task(
  "dev",
  gulp.series(gulp.parallel("clean", "lint:js"), "copy", "modernizr")
);
